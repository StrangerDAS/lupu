 ...
 * targetId: ID of user/vehicle
 * targetName: Optional name for display
 */
export default function ReportModal({ isOpen, onClose, targetType, targetId, targetName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Report submitted to Trust & Safety team', { id: toastId })
      onClose()
    } catch (err) {
  ...
  const reasonsList = targetType === 'user'
    ? ['Inappropriate Behavior', 'Fraudulent Activity', 'Fake Profile', 'Other']
    : ['Vehicle Condition Does Not Match', 'Unsafe Vehicle', 'Fake Listing', 'Other']
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold flex items-center gap-2 text-white">
                <FiAlertTriangle className="text-amber-500" /> Report {targetType === 'user' ? 'User' : 'Vehicle'}
              </h3>
            ...
            <p className="text-sm text-white/60 mb-6">
              You are reporting <strong>{targetName || 'this item'}</strong>. Our Trust & Safety team will review this report confidentially.
            </p>
